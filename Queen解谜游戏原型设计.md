...
棋盘为 n×n 网格。每个格子属于恰好一个颜色区域（四连通块：区域内的格子通过上下左右相邻连接，不可仅通过对角连接）。

```text
每行恰好 1 个 Queen
每列恰好 1 个 Queen
每个颜色区域恰好 1 个 Queen
任意两个 Queen 不能相邻（8 邻域 / 九宫格，含斜向和对角）
```
区域可跨行跨列但不规则延伸。四条规则构成 Queen 消元解谜的全部约束。
...
**生成器保证：**
- 每个生成的关卡有唯一解：求解器可在当前信息下唯一确定每个 Queen 的位置，不存在"某个 Queen 在合法解 A 中放在格子 X、在合法解 B 中放在格子 Y"的歧义。唯一解由求解器的确定性保证——给定同一棋盘，求解器始终输出同一组 Queen 位置。
...
```
#D9435F  红    #E8923A  橙    #D4B83D  黄
#47B86B  绿    #3BBFB6  青    #3A9FCA  蓝
#7B6CBF  紫    #D486A8  粉    #6AAAD4  淡蓝
#B8A67E  棕
```
...
- 格子尺寸：动态计算，桌面端上限 68px，移动端下限 24px，自适应视口。
- 格子圆角：7px。
- 网格间距：5px（gaps 显示深色棋盘背景）。
- 棋盘背景：深色（#181511），区域色块在其上。

### 5.4 Queen 渲染

使用 ♛ Unicode 字符（U+265B），深色文字，字号为格子尺寸的 76%。居中放置。Queen 格使用金色渐变背景。

### 5.5 X 标记渲染

使用 ✕ Unicode 字符，半透明深色，字号为格子尺寸的 64%。覆盖在区域颜色之上。错误标记（红 X）使用 ✕ 字符，白色文字，红色渐变背景。
...
|---|---|
| 默认 | 区域色块填充 + 对角线高光渐变 |
| 桌面端 hover | 饱和度提升 + 亮度提升 + 上浮阴影 |
| Queen 格 | 金色渐变底 + 深色 ♛ |
| X 标记格 | 区域色块降饱和度 + 半透明深色 ✕ |
| 唯一候选格 | 金色实线边框 + 外发光（常亮） |
| 求解器播放高亮格 | 白色内边框 + 青色外边框 + 外发光 |
...
L2_Lock2          红色系 (#b91432)
L2_Lock3          紫色系 (#4c39a0)
L3_Projection     蓝色系 (#006f9c)
L3_Contradiction  深橙系 (#a84316)
```
